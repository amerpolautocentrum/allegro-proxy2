...
module.exports = async (req, res) => {
    const { path } = req.query; // Pobieramy dynamiczny parametr 'path'
    const { offset = '0', limit = '6', sort = '-publication.start', ...filters } = req.query;
    
    // Budujemy URL z parametrem ścieżki i query string
    let url = `https://api.allegro.pl${path || '/sale/offers'}?offset=${offset}&limit=${limit}&sort=${sort}`;
